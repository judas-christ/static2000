  ...
  return function(options, onError) {
    ...
    for(var pluginName in modelPlugins) {
      if(pluginName in ContentModel.prototype) {
        //todo: warn that plugin is overwriting existing property and ill not be added
        continue;
      }
    ...
    return fs.src(path.join('**', options.templateAdapter.templateGlob), {cwd: options.content})
