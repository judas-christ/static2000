...
module.exports = function(state) {
    ...
    ContentModel.prototype = {
        ...
        descendants: function(filter) {
            var descs = state.contentList.filter(function(v) { return v.path.indexOf(this.path + '/') === 0 && v.path !== this.path; }, this); //this._children.map(function(v, i) { var kids = v._children.length ? v.descendants() : []; kids.push(v); console.log(kids); return kids; });
        ...
        relativePath: function(currentPath) {
            var relPath = path.relative(currentPath || state.model.path, this.path).replace(/\\/g, '/');
